...
3. Open the IDAgent logs (`docker compose logs idagent`) and copy the `wireguard public key:` line. Send it together with `DEPLOYMENT_NAME` and `SERVER_STATIC_IP` to Vereign (<kalin.canov@vereign.com>) so they can register your peer on the CA side.
4. After Vereign confirms registration, complete `/onboarding` in the dashboard to issue the S/MIME certificate.

For any **additional** peer (peer-to-peer between two Stargates), exchange public keys + endpoints with the other party and add the connection through the IDAgent API:
