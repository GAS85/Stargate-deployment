...
!!! important "Rebuilding the Postfix relay image"
    The `postfix-relay` service is built locally from `config/postfix/Dockerfile` and `wrapper.sh` - it is **not** pulled from a registry. If a `git pull` brings in changes to those files, `docker compose pull` will not pick them up. Rebuild and recreate the container explicitly:
